...
class GamePlayPlayerChannelTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function getResponse(
        ...
        bool $playerHackHost = false,
    ): TestResponse
    {
        $response = $this->withHeader('X-Requested-With', 'XMLHttpRequest');

        if ($user !== false) {
            $response = $this->actingAs($user ?? ($playerHackHost ? $this->player : $this->host), 'web');
        }
        ...
        return $response->post($uri, [
            ...
            'channel_name' =>
                ...
                . '.' . ($user === false ? '' : $user?->getId() ?? $this->host->getId()),
        ]);
    }
